...
import { convertStringToNumbersArray } from "../js4-array-iterators/challenge";
...
export const totalScoresArr = (scoresArr) =>
  scoresArr.reduce((acc, curr) => acc + curr, 0);
/**
...
export const reverseString = (toReverse) => {
  // reverse can only be used on an array
  const tmp = toReverse.split("");
  const tmp2 = tmp.reverse();
  const final = tmp2.join("");
  return final;
};
...
export const sortCharactersAlphabetically = (charcterArr) =>
  charcterArr.join("").toLowerCase().split("").sort();
...
// check graphite and mdn for more info
...
export const checkItemInstock = (toCheck) => {
  ...
  for (let i = 0; i < stockList.length; i++) {
    if (stockList[i] === toCheck) {
      tmp += i;
    }
  }

  if (tmp) {
    return `${toCheck} is instock, it is on aisle ${tmp}.`;
  } else {
    return `Sorry ${toCheck} is not instock.`;
  }
};

// // check stockList for item
// const check = stockList.filter((value, index) =>
//   stockList[index].includes(value)
// );
...
// // if item is there then pass strings

// function checkList (toCheck, )

// // filter to look through
// function filter(toCheck, index, stockList) {
//   stockList[index] = toCheck;
// }

// return toCheck.filter(check)
//   ? `${toCheck} is instock, it is on aisle ${index}.`
//   : `Sorry ${toCheck} is not instock.`;
...
export const checkPrimaryColours = (coloursArr) => {
  const checkPrimary = (arr) =>
    arr.includes("red") || arr.includes("blue") || arr.includes("yellow");

  return coloursArr.every(checkPrimary);
};
 ...
 * @return {boolean} t r u e
 * e u r t
 */

export const checkStringPalindrome = (stringOne) => {
  const tmp = stringOne.split("").reverse().join("");

  return tmp === stringOne;
};
...
export const totalNestedScoresArr = (scoresArr) => {
  ...
  for (let i = 0; i < scoresArr.length; i++) {
    tmp.push(scoresArr[i].reduce((acc, curr) => acc + curr, 0));
  }
  return tmp;
};
