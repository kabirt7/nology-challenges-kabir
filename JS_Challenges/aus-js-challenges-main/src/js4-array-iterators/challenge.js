...
export const createPercentageList = (numbersArr) =>
  numbersArr.map((value) => value * 100 + "%");

// {
//   const percentageList = numbersArr.map(function (value) {
//     return value * 100 + "%";
//   });
...
export const createListOfPoessessions = (possessionsArr, name) =>
  possessionsArr.map((value) => `${name} ${value}`);
//   return name + " " + value;

//   const listOfPoessessions = possessionsArr.map(function (value) {
//     return name + " " + value;
//   });
...
export const convertStringToNumbersArray = (numberString) => {
  ...
  const finishedArray = newArray.map((value) => Number(value));
...
export const createOddEvenArray = (numberString) => {
  // turn string of numbers into an array of numbers using split to remove +'s
  const newArray = numberString.split("+");

  const finalArray = newArray.map((value) =>
    value % 2 === 0 ? "even" : "odd"
  );
...
// map each into a boolean value
// const finalArray = newArray.map(function (value) {
//   if (value % 2 === 0) {
//     return "even";
//   } else {
//     return "odd";
...
export const filterBooksBySearch = (booksArr, searchTerm) =>
  booksArr.filter((value) => value.includes(searchTerm));
// {
//   // filter books depending on if they have the search term
//   const newArray = booksArr.filter((value) => value.includes(searchTerm));
...
export const formatString = (string) => {
  ...
  const lettersOnly = string.replace(/[^a-zA-Z]/g, "");

  // split into separate array elements
  const newArray = lettersOnly.split("");

  // if even (or 0) turn string uppercase else lowercase
  const finalArray = newArray.map((value, index) =>
...
export const fizzBuzz = (mixedArray) => {
  const newArray = mixedArray.filter((value) => value > 0);
