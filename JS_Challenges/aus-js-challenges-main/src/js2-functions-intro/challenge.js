...
export const celsiusToFahrenheit = (tempInCelsius) => {
  const tempInFahrenheit = (9 / 5) * tempInCelsius + 32;
  const answer = `${tempInCelsius} degrees celsius is ${tempInFahrenheit} degrees fahrenheit`;
  return answer;
};
...
export const calculateLifetimeSupply = (snickersPerDay, age, maxAge) => {
  const totalSnickers = snickersPerDay * (maxAge - age) * 365;
...
export const calculateAreaOfCircle = (radius) => {
  const circleArea = parseFloat((Math.PI * radius ** 2).toFixed(2));
  return circleArea;
};
...
export const getStudentSummary = (score, name) => {
  if (typeof score !== "number" || score < 0 || score > 100)
    if (score >= 80) {
      return `"A": "Congratulations ${name}! You achieved a grade of A."`;
    } else if (score >= 70) {
      return `"B": "Congratulations ${name}! You achieved a grade of B."`;
    } else if (score >= 60) {
      return `"C": "Congratulations ${name}! You achieved a grade of C."`;
    } else if (score >= 50) {
      return `"D": "Congratulations ${name}! You achieved a grade of D."`;
    } else if (score >= 40) {
      return `"E": "Congratulations ${name}! You achieved a grade of E."`;
    } else if (score >= 0 && score < 40) {
      return `"F": "Congratulations ${name}! You achieved a grade of F."`;
    } else {
      return "My apologies John, there's been an error in processing your grade.";
    }
};
